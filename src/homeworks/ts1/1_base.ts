...
type TNumberArray = {value: number , number: number};

export const getNumberedArray = (arr: number[]): TNumberArray[] => arr.map((value, number) => ({ value, number }));
export const toStringArray = (arr: TNumberArray[]): string[] => arr.map(({ value, number }) => `${value}_${number}`);
...
type TCustomerReturn = Record<number, Omit<Customer, 'id'>>;
  // [id: number]: Omit<Customer, 'id'>


export const transformCustomers = (customers: Customer[]): TCustomerReturn => {
  return customers.reduce((acc: Record<number, Omit<Customer, 'id'>>, customer: Customer) => {
